  ...
  if (na_tray_child_has_alpha (NA_TRAY_CHILD (widget)))
    {
      ...
      /* begin fix for lxpanel2 */
      /* NOTE: the original NaTray in gnome-panel does not do this
       * However, in lxpanel2 we use a non-windowed widget.
       * So the cairo context is actaully from parent widget, the panel.
       * allocation.x and .y of NaTrayChild are relative to the top most
       * panel widget, not NaTray. So we need to convert the coordinate
       * to make them relative to NaTray widget, so cairo clip can work
       * correctly. */
