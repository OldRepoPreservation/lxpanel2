...
public class TaskListApplet : Gtk.Box, Applet, Gtk.Orientable {
	...
	public TaskListApplet(Panel panel) {
		...
		window_hash = new HashTable<weak Wnck.Window, weak TaskButton>(direct_hash, direct_equal);
	...
	private void on_window_closed(Wnck.Screen screen, Wnck.Window window) {
		...
		unowned TaskButton? btn = window_hash.lookup(window);
		if(btn != null) {
			btn.destroy();
			window_hash.remove(window);
		}
	}

	public void on_active_window_changed(Wnck.Screen screen, Wnck.Window? prev_active) {
		unowned TaskButton? btn = window_hash.lookup(prev_active);
		...
		btn = window_hash.lookup(screen.get_active_window());
		if(btn != null) {
	...
	public void on_active_workspace_changed(Wnck.Screen screen, Wnck.Workspace? prev) {
		...
		var it = HashTableIter<weak Wnck.Window, weak TaskButton>(window_hash);
	...
	protected override void size_allocate(Gtk.Allocation allocation) {
		...
		foreach(var child in children) {
			child.set_size_request(btn_size, btn_height);
			grid.attach(child, col, row, 1, 1);
			++col;
			if(col >= n_cols) {
				col = 0;
				++row;
...
*/
	...
	private HashTable<weak Wnck.Window, weak TaskButton> window_hash;
	private HashTable<weak Wnck.Application, weak TaskAppButton> application_hash;
	private weak Panel panel;
}
