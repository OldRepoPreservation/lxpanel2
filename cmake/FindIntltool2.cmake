...
macro (INTLTOOL_CREATE_TRANSLATIONS2)
	...
	# for *.pot file
	set (pot_name "${GETTEXT_PACKAGE}.pot")
	set (pot_path "${IT_PO_DIRECTORY}/${pot_file_name}")
	...
	if (IT_ALL_LINGUAS)
		...
		file(GLOB po_file_names RELATIVE ${IT_PO_DIRECTORY} "*.po")
		set (IT_ALL_LINGUAS "")
    ...
    add_custom_command(
        OUTPUT ${pot_path}
        COMMAND ${INTLTOOL_UPDATE_EXECUTABLE} --pot --gettext-package ${IT_GETTEXT_PAKCAGE}
    ...
    set(update_po_target "${IT_GETTEXT_PACKAGE}_update-po")
    add_custom_target (${update_po_target}
        DEPENDS ${pot_path}
    )
    add_dependencies(update-po ${update_po_target})

endmacro (INTLTOOL_CREATE_TRANSLATIONS2)


macro (INTLTOOL_CREATE_TRANSLATIONS _gettext_package)

	# keep the generated files from "make clean"
	set_property(DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR} PROPERTY CLEAN_NO_CUSTOM true)

    set (linguas_file  "${CMAKE_CURRENT_SOURCE_DIR}/LINGUAS")
    if (EXISTS ${linguas_file})
        # read LINGUAS and build a list of po files according to its content
        file(STRINGS ${linguas_file} linguas)
        set (po_files "")
        foreach(lang ${linguas})
            list (APPEND po_files "${lang}.po")
        endforeach(lang)
    else(EXISTS ${linguas_file})
        # if LINGUAS file is not available, let's generate the list of po 
        # files on the fly
        file(GLOB po_files RELATIVE ${CMAKE_CURRENT_SOURCE_DIR} "*.po")
    endif(EXISTS ${linguas_file})

    set (pot_file_path "${CMAKE_CURRENT_SOURCE_DIR}/${_gettext_package}.pot")

    # add rules for updating the pot file
    set (pot_src_files "")
    # read the content of POTFILES.in and add the files listed in it
    # to the dependencies of the *.pot file.
    set (potfiles_in_file_path  "${CMAKE_CURRENT_SOURCE_DIR}/POTFILES.in")
    if (EXISTS ${potfiles_in_file_path})
        # read POTFILES.in file
        file(STRINGS ${potfiles_in_file_path} potfiles_in_lines)
        foreach (line ${potfiles_in_lines})
            # omit lines starts with #
            if (NOT ${line} MATCHES "^#.*")
                # need to remove things like [type: gettext/glade]
                # regexp for it: \s*\[type:[^]]*\]\s*
                string (REGEX REPLACE "[ \\t]*\\[type:[^]]*\\][ \\t]*" "" pot_src_file ${line})
                set (pot_src_file "${CMAKE_SOURCE_DIR}/${pot_src_file}")
                list (APPEND pot_src_files ${pot_src_file})
            endif (NOT ${line} MATCHES "^#.*")
        endforeach (line)
    endif (EXISTS ${potfiles_in_file_path})

    # add a custom command to build the pot file
    add_custom_command(
        OUTPUT ${pot_file_path}
        COMMAND ${INTLTOOL_UPDATE_EXECUTABLE} --pot --gettext-package ${_gettext_package}
        WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
        DEPENDS ${potfiles_in_file_path} ${pot_src_files}
    )

    # add build commands for po files which generate *.gmo files and
    # merge translated *.po files with the pot file.
    set (gmo_file_paths "")
    foreach (po_file_name ${po_files})
        get_filename_component(po_file_path ${po_file_name} ABSOLUTE)
        get_filename_component(lang ${po_file_name} NAME_WE)

        # due to some limitations of CMake gettext module, we did not
        # use its GETTEXT_CREATE_TRANSLATIONS macro but made our own.
        set(gmo_file_path ${CMAKE_CURRENT_BINARY_DIR}/${lang}.gmo)
        add_custom_command(
            OUTPUT ${gmo_file_path}
            COMMAND ${INTLTOOL_UPDATE_EXECUTABLE} --dist ${lang} --gettext-package ${_gettext_package}
            # COMMAND ${GETTEXT_MSGMERGE_EXECUTABLE} --quiet --update --backup=none -s ${_absFile} ${_absPotFile}
            COMMAND ${GETTEXT_MSGFMT_EXECUTABLE} -o ${gmo_file_path} ${po_file_path}
            WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}
            DEPENDS ${pot_file_path} ${po_file_path}
        )
        list (APPEND gmo_file_paths ${gmo_file_path})
    ...
    set(update_po_target "${_gettext_package}_update-po")
    # FIXME: should we do it with ALL?
    add_custom_target (${update_po_target}
        DEPENDS ${pot_file_path} ${gmo_file_paths}
    )
